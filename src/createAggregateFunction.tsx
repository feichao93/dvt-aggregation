...
function emit(items: Item[], context: { indicators: IndicatorConfig[] }) {
  ...
  return [
    ...
    t.____(...context.indicators.map(ind => `${ind.name}: ${ind.code} = ${ind.expression}`)),
    t.__('*/'),
    '',
...
export default function createAggregateFunction(
  indicators: IndicatorConfig[],
  // TODO 添加是否开启严格模式的选项
  { skipEmptyExpression = true }: { skipEmptyExpression?: boolean } = {},
): (slice: any[]) => any {
  const context = { indicators }

  let filteredIndicators = indicators
  if (skipEmptyExpression) {
    filteredIndicators = indicators.filter(ind => Boolean(ind.expression))
  }
  const template = emit(topologicalSort(convertItems(parseIndicators(filteredIndicators))), context)
